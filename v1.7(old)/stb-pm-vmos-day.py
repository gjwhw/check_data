...
while loop:
    try:
        stb_pm_vmos_chunk = stb_pm_vmos.get_chunk(chunkSize)
        ...
        sum = sum.round(6)
        # 按照3列进行拼接
        z = pd.merge(avg, sum, on=['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'])
...
# 再次求平均值
avg = df.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['MDI-DF平均值',
                        'MDI-MLR平均值', 'RTP丢包率平均值', 'MDI-DF劣化百分比', 'MDI-MLR劣化百分比', 'RTP丢包率劣化百分比', '收视vmos'].mean()
avg = avg.round(6)
# 再次求和
sum = df.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['卡顿总时长',
						'收视总时长', '卡顿总次数', '卡顿超过10秒的总次数'].sum()
sum = sum.round(6)
# 按照3列进行拼接
