...
x = pd.read_csv(filename, iterator=True, names=name1, sep='|', header=None)

name2 = ['TIMESTAMP', '省地区代码', '市地区代码', 'STB_ID', '机顶盒IP', '业务账号', '接入方式', '机顶盒SN序列号',
		 '机顶盒MAC', '厂商名称', '机顶盒型号', '机顶盒软件版本号', '机顶盒硬件版本号',]
y2 = pd.read_csv(filename2,iterator=True, names=name2, sep='|', header=None)
y1 = y2.get_chunk()
y1 = y1.loc[:,['STB_ID','厂商名称', '机顶盒型号']]
...
while loop:
    try:
        chunk1 = x.get_chunk(chunkSize)
        # 第一列时间格式为：‘2018-03-22 00:00：00’，取前13位进行分割，后面实现按小时分组：‘2018-03-22 00’
        chunk1['TIMESTAMP'] = chunk1['TIMESTAMP'].astype('str').str[:13]

        chunk = pd.merge(chunk1, y1, on=['STB_ID'])

        # 求平均数的指标
        y = chunk.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['MDI-DF平均值',
                        'MDI-MLR平均值', 'RTP丢包率平均值', 'MDI-DF劣化百分比', 'MDI-MLR劣化百分比', 'RTP丢包率劣化百分比', '收视vmos'].mean()
        # 小数点保留6位
        y = y.round(6)
        # 求和
        n = chunk.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['卡顿总时长',
						'收视总时长', '卡顿总次数', '卡顿超过10秒的总次数'].sum()
        n = n.round(6)
        # 按照3列进行拼接
        z = pd.merge(y, n, on=['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'])
...
# 再次求平均值
y = df.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['MDI-DF平均值',
                        'MDI-MLR平均值', 'RTP丢包率平均值', 'MDI-DF劣化百分比', 'MDI-MLR劣化百分比', 'RTP丢包率劣化百分比', '收视vmos'].mean()
y = y.round(6)
# 再次求和
n = df.groupby(['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'], as_index=False)['卡顿总时长',
						'收视总时长', '卡顿总次数', '卡顿超过10秒的总次数'].sum()
n = n.round(6)
# 按照3列进行拼接
z = pd.merge(y, n, on=['TIMESTAMP', '省地区代码', '市地区代码','厂商名称', '机顶盒型号'])
